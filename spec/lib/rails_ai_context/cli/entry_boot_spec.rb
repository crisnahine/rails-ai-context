...
RSpec.describe RailsAiContext::CLI::EntryBoot do
  ...
  describe ".call" do
    ...
    it "degrades a failed boot to :static when static is allowed, and to :absent when it is not" do
      Dir.mktmpdir do |dir|
        FileUtils.mkdir_p(File.join(dir, "config"))
        File.write(File.join(dir, "config/environment.rb"), "raise 'boom'\n")
        allow(RailsAiContext::BootManager).to receive(:boot!).and_return(
          RailsAiContext::BootManager::Result.new(status: :failed, error: RuntimeError.new("boom"))
        )
