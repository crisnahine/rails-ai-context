  ...
  module Tools
    class GetMailers < BaseTool
      ...
      def self.call(mailer: nil, offset: 0, limit: nil, server_context: nil)
        ...
        return text_response("Mailer introspection not available. Add :jobs to introspectors.") unless jobs
        return text_response("Mailer introspection failed: #{jobs[:error]}") if jobs[:error]
        note = unavailable_note(jobs)
